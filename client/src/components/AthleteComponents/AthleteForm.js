...
const AthleteForm = (props) => {
  ...
  var [testNames, setTestNames] = useState();
  requests.post("/user/username", { username: "tests" }).then((res) => {
    setTestNames(res.data[0]);
  });
  ...
  const checkArrow = () => {
    ...
    if (moment().toDate().getMonth() === moment(today).toDate().getMonth())
      setCanNext({ color: "grey" });
  };
  const hideFitness = () => {
  ...
  const goToFillForm = (id) => {
    ...
    if (
      (parseInt(todayArray[0]) < parseInt(theDayForFormArray[0]) &&
        parseInt(todayArray[1]) === parseInt(theDayForFormArray[1])) ||
      (parseInt(todayArray[1]) < parseInt(theDayForFormArray[1]) &&
        todayArray[2] === theDayForFormArray[2])
    ) {
      return;
    }
    requests.post("/form/checkforform", info).then((res) => {
      if (res.data) alert(res.data);
      else setFillForm(!fillForm);
    });
  };
    ...
    if (user.username) {
      ...
      datess.map((value) => {
        if (moment(value).isAfter(moment()))
          trainingDates.push(moment(value).format("DD/MM/YY"));
      });
    }
  };
  const fillFormColor = (e) => {
    if (trainingDates.includes(e.target.id))
      setFillFormColorState({ background: "grey" });
    ...
    <div className="athlete-form">
      {!fillForm ? (
        <div className="athlete-form-header">
          ...
          <img
            className="athlete-form-player-photo"
            src={item.image != " " ? item.image : imag}
          ></img>
          ...
          <div className="month-form">
            ...
            <div className="fill-form-field"></div>
          </div>
              ...
              {datess.map((day, index) => {
                var conditioner = true;
                {
                  ...
                  if (user.training)
                    user.training.map((value) => {
                      if (
                        value.date == moment(day).format("DD/MM/YY") &&
                        value.session2 === " "
                      ) {
                        styleForDot = formFilledPartially;
                        conditioner = false;
                      }
                    });
                }
                return (
                  <div
                    ...
                    onClick={(e) => {
                      setTheDayForForm(e.target.id);
                      fillFormColor(e);
                      conditioner
                        ? goToFillForm(e.target.id)
                        : fillSecondSession();
                    }}
                    key={index}
                    style={determineToday(day)}
                    className="date-day"
                  >
          ...
          <div className="body-fitness-training">
                  ...
                  {numberOfExercises.map((el, index) => {
                    return (
                      <div key={index} className="athlete-training-values">
                        <label
                          key={`${index}` + "1"}
                          className="athlete-training-value"
                        >
                          value
                        </label>
                        <label
            ...
            <div className="athlete-body-measurements">
              <label className="athlete-training-title">
                Body measurements
              </label>
              {bodyHidden ? (
                <div className="athlete-body-measurements-attributes">
                  ...
                  <div className="athlete-body-measurements-values">
                    ...
                    >
                      {user.measurements
                        ? user.measurements[user.measurements.length - 1].height
                        : ""}{" "}
                      m
                    </label>
                    <label
                    ...
                    >
                      {user.measurements
                        ? user.measurements[user.measurements.length - 1].fat
                        : ""}{" "}
                      %
                    </label>
              ...
              {fitnessHidden ? (
                <div className="athlete-body-measurements-attributes">
                  <div className="athlete-body-measurements-labels">
                    ...
                    {testNames ? (
                      <label className="athlete-body-measurements-label">
                        {testNames.tests[0].test8}
                      </label>
                    ) : (
                      ""
                    )}
                 </div>
                 <div onClick={()=>{console.log(user.tests)}} className="athlete-body-measurements-values">
                    <label className="athlete-body-measurements-value">
                      {user.tests?user.tests[0].test1:""}
                    </label>
                    <label className="athlete-body-measurements-value">
                      {user.tests?user.tests[0].test2:""}
                    </label>
                    <label className="athlete-body-measurements-value">
                      {user.tests?user.tests[0].test3:""}
                    </label>
                    <label className="athlete-body-measurements-value">
                      {user.tests?user.tests[0].test4:""}
                    </label>
                    <label className="athlete-body-measurements-value">
                      {user.tests?user.tests[0].test5:""}
                    </label>
                    <label className="athlete-body-measurements-value">
                      {user.tests?user.tests[0].test6:""}
                    </label>
                    <label className="athlete-body-measurements-value">
                      {user.tests?user.tests[0].test7:""}
                    </label>
                    <label className="athlete-body-measurements-value">
                      {user.tests?user.tests[0].test8:""}
                    </label>
                  </div>
                </div>
              ) : (
      ...
      {fillForm ? (
        <AthleteFormActual
          ...
          fillForm={setFillForm}
          user={user}
          theDayForForm={theDayForForm}
        />
      ) : (
        ""
      )}
